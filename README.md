...
| **v2.1.5** | 2026-05 | **修复发行包路径问题** — Web 前端路径改为基于可执行文件定位；修复 Makefile brace expansion 打包问题 |
| **v2.1.4** | 2026-05 | **修复置数功能三大问题** — 置数框缺少 @input 事件、DI 开关状态不更新、置数与策略冲突导致禁用；新增手动置数策略 UI 支持 |
...
- **CSV 多测点同步回放** — 详情页新增独立"CSV 多测点回放"卡片
  - 上传多列 CSV 文件（time,value1,value2...），每列映射到不同测点
  - 支持相对时间（ms/s）和绝对时间（hh:mm:ss）两种回放模式
  - 多个测点共享同一时间基准，严格同步回放
  - 状态持久化：退出详情页再打开自动恢复运行状态
  - 向后兼容：单测点 CSV 模式完全不变
- **CSV 回放时序修复** — 基于 elapsed time 控制回放节奏，修复原逻辑一次性回放全部行的问题
- **Modbus TCP 协议支持** — 与 IEC104 并行运行，支持功能码 1/3/4/5/6/16

### v2.2.0 新特性

- **自定义公式自动变化策略** — 新增第11种策略 `custom`，支持通过按钮式编辑器构造四则运算公式
  - 支持运算符：`+`、`-`、`*`、`/`、`(`、`)`
  - 支持 2~50 个关联测点，使用 `{0}`、`{1}`... 作为占位符
  - 递归下降表达式解析器，支持括号优先级和除零保护
- **前端公式编辑器** — 弹窗中新增"自定义公式"Tab：
  - 远程搜索多选关联测点
  - 按钮式公式构造（运算符按钮 + 参数按钮）
  - 公式实时预览
  - 计算周期配置（≥100ms）
- **CSV 导入导出支持** — 策略代码 `11`，A列为IOA列表，B列为公式，C列为周期
